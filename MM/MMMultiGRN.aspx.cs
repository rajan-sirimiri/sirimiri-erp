...
namespace MMApp
{
    public partial class MMMultiGRN : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!MMDatabaseHelper.RoleHasModuleAccess(role, "MM", "MM_RM_GRN"))
            ...
            if (!IsPostBack)
            {
                LoadSuppliers();
                LoadPendingInvoices();
                // GRN history defaults: last 30 days
        ...
        void BuildRMOptionsJson()
        {
            DataTable dt = MMDatabaseHelper.GetActiveRawMaterials();
            var sb = new StringBuilder("[");
            foreach (DataRow r in dt.Rows)
            {
                string name = EscapeJson(r["RMName"].ToString());
                sb.AppendFormat("{{\"id\":\"{0}\",\"name\":\"{1}\"}},", r["RMID"], name);
            }
            if (sb.Length > 1) sb.Length--;
            sb.Append("]");
        ...
        // ── GRN History (multi-item RM GRNs only) ────────────────────────
        void LoadGRNList()
        {
            try
            {
                ...
                DataTable dt = MMDatabaseHelper.GetMultiItemRawInwardList(from, to);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Returns "MGRN-RM-00001" format. Independent counter from single-item GRNs.
                string grnNo = MMDatabaseHelper.GenerateMultiGRNNumber("RM");
