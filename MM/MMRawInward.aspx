                    ...
                    <asp:Repeater ID="rptGRN" runat="server">
                        ...
                        <ItemTemplate>
                            <tr class="grn-row" data-grn='<%# Eval("GRNNo") %>'>
                                <td>
                                    <a href="javascript:void(0)" class="grn-no-link" onclick="toggleGRNDetail(this,'<%# Eval("GRNNo") %>','RM');">
...
<script>
    ...
    function toggleGRNDetail(linkEl, grnNo, tab) {
        var chev    = linkEl.querySelector('.chev');
        var trLink  = linkEl.closest('tr');
        if (!trLink) return;
        // Find the matching detail row immediately after
        ...
        // Fetch on first open only; re-use the rendered content thereafter
        ...
        var url = 'MMGRNDetailAPI.ashx?tab=' + encodeURIComponent(tab) + '&grn=' + encodeURIComponent(grnNo) + '&_=' + Date.now();
        ...
        xhr.onreadystatechange = function() {
            ...
            if (xhr.status !== 200) {
                cell.innerHTML = '<div class="grn-detail-err">Failed to load GRN details (HTTP ' + xhr.status + ').</div>';
                return;
            }
            try {
                var data = JSON.parse(xhr.responseText);
                if (data.error) {
                    cell.innerHTML = '<div class="grn-detail-err">' + escapeHtml(data.error) + '</div>';
                    return;
                }
                cell.innerHTML = renderGRNDetail(data);
                cell.setAttribute('data-loaded', '1');
            } catch (e) {
                cell.innerHTML = '<div class="grn-detail-err">Error parsing response.</div>';
            }
        };
        xhr.send();
    }
    ...
    function renderGRNDetail(d) {
        ...
        var html = '<div class="grn-detail-content">';

        // Header card (invoice / PO / supplier / created)
        html += '<div class="grn-detail-header">';
        ...
        if (isMulti) {
            // Multi-item: table of all line items
            html += '<table class="grn-lines-table"><thead><tr>' +
                    '<th>#</th><th>Material</th><th>HSN</th>' +
            ...
            for (var i = 0; i < d.lines.length; i++) {
                ...
                html += '<tr>';
                html += '<td>' + (i + 1) + '</td>';
                html += '<td><div style="font-weight:500;">' + escapeHtml(L.matName) + '</div>' +
                        '<div style="font-size:10px;color:#999;">' + escapeHtml(L.matCode) + '</div></td>';
                ...
                html += '<td style="text-align:right;">' +
                        (parseFloat(L.shortageQty) > 0
                            ? '<span class="badge-shortage">' + fmtNum(L.shortageQty) + '</span>'
                            : '—') + '</td>';
                html += '<td style="text-align:right;font-weight:600;">' + fmtRs(L.amount) + '</td>';
                html += '</tr>';
            }
            html += '</tbody></table>';
        } else if (d.lines && d.lines.length === 1) {
            // Single-item: key/value grid with all fields
            var L = d.lines[0];
            ...
            html += '</div>';
            if (L.remarks) {
                html += '<div style="margin-top:10px;padding:10px 14px;background:#fff;border:1px solid var(--border);border-radius:8px;font-size:12px;">' +
                        '<span style="font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--text-dim);">Remarks</span><br/>' +
    ...
    function hdrCell(lbl, val) {
        return '<div><span class="label">' + escapeHtml(lbl) + '</span><span class="value">' + val + '</span></div>';
    }
    function kv(lbl, valHtml) {
        return '<div><span class="label">' + escapeHtml(lbl) + '</span><span class="value">' + valHtml + '</span></div>';
    }
    function fmtNum(v) {
        var n = parseFloat(v);
        if (isNaN(n)) return '0';
