<%@ Page Language="C#" AutoEventWireup="true" Inherits="MMApp.MMOpeningStockUpload" EnableEventValidation="false" %>
<!DOCTYPE html><html lang="en"><head runat="server">
<meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Opening Stock Upload &mdash; MM</title>
...
.row-ok{color:var(--teal);}.row-err{color:var(--red);}
...
<div class="page-header">
    <div class="page-title">Opening Stock <span>Upload</span></div>
    <div class="page-sub">Upload opening stock with price data for Raw Materials, Packing Materials, Consumables, and Stationaries via Excel</div>
</div>
<div class="main">
    ...
    <div class="card">
        ...
        <p style="font-size:13px;color:var(--text-muted);margin-bottom:12px;">
            Download the Excel template, fill in the data for each material type (one sheet per type), then upload.
            The template has 4 sheets: Raw Materials, Packing Materials, Consumables, Stationaries.
        </p>
        <p style="font-size:12px;color:var(--text-dim);margin-bottom:14px;">
            <strong>Required columns:</strong> Material Code, Quantity, Rate, As Of Date (DD-MM-YYYY).
            Material Name is for your reference only — the system matches by Material Code.
        </p>
    ...
    <div class="card">
        ...
        <div class="upload-area">
            <asp:FileUpload ID="fuExcel" runat="server" accept=".xlsx,.xls" style="font-size:13px;"/>
    ...
    <div class="card">
        ...
        <!-- Tab buttons -->
        <div class="tab-row">
            <asp:Button ID="btnTabRM" runat="server" Text="Raw Materials" CssClass="tab-btn active" OnClick="btnTab_Click" CommandArgument="RM" CausesValidation="false"/>
        ...
        <asp:HiddenField ID="hfActiveTab" runat="server" Value="RM"/>
        ...
        <asp:Repeater ID="rptPreview" runat="server">
            <HeaderTemplate><table><thead><tr><th>Row</th><th>Code</th><th>Name</th><th class="num">Qty</th><th class="num">Rate</th><th>Date</th><th>Remarks</th><th>Status</th></tr></thead><tbody></HeaderTemplate>
            <ItemTemplate><tr>
                <td><%# Eval("RowNum") %></td>
                <td style="font-weight:600;"><%# Eval("Code") %></td>
                <td><%# Eval("Name") %></td>
                <td class="num"><%# Eval("Qty") %></td>
                <td class="num"><%# Eval("Rate") %></td>
                <td><%# Eval("DateStr") %></td>
                <td style="font-size:11px;"><%# Eval("Remarks") %></td>
                <td class='<%# Eval("IsValid").ToString()=="True" ? "row-ok" : "row-err" %>' style="font-weight:700;font-size:11px;"><%# Eval("StatusMsg") %></td>
            </tr></ItemTemplate>
            <FooterTemplate></tbody></table></FooterTemplate>
        </asp:Repeater>

        <div class="btn-row">
            <asp:Button ID="btnConfirmUpload" runat="server" Text="&#x2705; Confirm &amp; Save All" CssClass="btn btn-success" OnClick="btnConfirmUpload_Click" CausesValidation="false"/>
        </div>
    </div>
    </asp:Panel>
</div>
