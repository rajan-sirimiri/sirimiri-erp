...
namespace MMApp
{
    public partial class MMMultiGRNCN : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!MMDatabaseHelper.RoleHasModuleAccess(role, "MM", "MM_CN_GRN"))
        ...
        void BuildItemJson()
        {
            ...
            foreach (DataRow r in dt.Rows)
            {
                ...
                sb.AppendFormat("\"{0}\":{{\"hsn\":\"{1}\",\"gst\":\"{2}\",\"uom\":\"{3}\",\"uomId\":\"{4}\"}},",
                    r["ConsumableID"], hsn, gst, uom, uomId);
            }
        ...
        {
            DataTable dt = MMDatabaseHelper.GetActiveConsumables();
            var sb = new StringBuilder("[");
            foreach (DataRow r in dt.Rows)
            {
                string name = EscapeJson(r["ConsumableName"].ToString());
                sb.AppendFormat("{{\"id\":\"{0}\",\"name\":\"{1}\"}},", r["ConsumableID"], name);
            }
            if (sb.Length > 1) sb.Length--;
            sb.Append("]");
        ...
        {
            DataTable dt = MMDatabaseHelper.GetPendingInvoiceCN();
            if (dt.Rows.Count > 0)
            {
        ...
        void LoadGRNList()
        {
            ...
            DataTable dt = MMDatabaseHelper.GetMultiItemConsumableInwardList(from, to);
            ...
            try
            {
                ...
                foreach (var item in payload.Items)
                {
                    ...
                    MMDatabaseHelper.AddConsumableInward(
                        ...
                        payload.InvoiceNo, supplierId, consumableId,
