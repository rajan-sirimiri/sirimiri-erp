...
<head runat="server">
    ...
    <title>Stationary &amp; Other Inward &mdash; MM</title>
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600&display=swap" rel="stylesheet" />
    <link rel="stylesheet" href="/StockApp/erp-tablet.css"/>
    <style>
        :root { --bg:#f5f5f5; --surface:#fff; --border:#e0e0e0; --accent:#cc1e1e; --teal:#1a9e6a; --primary:#4a7fa5; --gold:#b8860b; --orange:#e07b00; --text:#1a1a1a; --text-muted:#666; --text-dim:#999; --radius:12px; }
        *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .qc-box { background:rgba(74,127,165,0.04); border:1.5px solid rgba(74,127,165,0.2); border-radius:8px; padding:12px 16px; }
        ...
        .btn-receive:hover { background:#376590; box-shadow:0 4px 12px rgba(74,127,165,0.3); }
        ...
        .alert-success { background:rgba(74,127,165,0.10); color:var(--primary); border:1px solid rgba(74,127,165,0.25); }
        .alert-danger  { background:rgba(204,30,30,0.08); color:var(--accent); border:1px solid rgba(204,30,30,0.2); }
        .rec-panel { background:var(--surface); border:1.5px solid rgba(74,127,165,0.3); border-radius:var(--radius); overflow:hidden; position:sticky; top:68px; }
        .rec-header { background:rgba(74,127,165,0.08); padding:14px 18px; border-bottom:1px solid rgba(74,127,165,0.2); }
        .rec-title { font-family:'Bebas Neue',sans-serif; font-size:17px; letter-spacing:.07em; color:var(--primary); }
        ...
        .rec-chip-qty   { background:rgba(74,127,165,0.10); color:var(--primary); }
        .rec-chip-value { background:rgba(204,30,30,0.08); color:var(--accent); }
        .rec-total { padding:12px 18px; border-top:2px solid rgba(74,127,165,0.2); background:rgba(74,127,165,0.05); display:flex; justify-content:space-between; align-items:center; }
        .rec-total-label { font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color:var(--primary); }
        .rec-total-val { font-family:'Bebas Neue',sans-serif; font-size:20px; color:var(--primary); }
        ...
        .badge-qc-pass  { display:inline-block; padding:2px 6px; border-radius:20px; font-size:10px; font-weight:700; background:rgba(74,127,165,0.10); color:var(--primary); }
...
<form id="form1" runat="server">
    ...
    <nav>
        ...
        <span class="nav-sep">&rsaquo;</span>
        <span class="nav-item active">Stationary &amp; Other Inward</span>
        <div class="nav-right">
    ...
    <div class="page-header">
        <div>
            <div class="page-title">Stationary &amp; Other <span>Inward</span></div>
            <div class="page-sub">Record stationery and miscellaneous item receipts &mdash; generate GRN</div>
        </div>
        <div class="grn-badge"><asp:Label ID="lblGRNNo" runat="server" Text="GRN-ST-#####" /></div>
    </div>
        ...
        <div>
            ...
            <div class="card">
                ...
                <div class="form-grid">
                    <div class="form-group" style="position:relative;">
                        <label>Stationary / Other Item <span class="req">*</span></label>
                        <input type="text" id="txtItemSearch" placeholder="Type to search item..."
                            oninput="filterDropdown(this.value, '<%= ddlItem.ClientID %>', 'txtItemSearch');"
                            onfocus="filterDropdown(this.value, '<%= ddlItem.ClientID %>', 'txtItemSearch');"
            ...
            <div class="card">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group" style="background:rgba(74,127,165,0.06);border:1.5px solid rgba(74,127,165,0.3);border-radius:10px;padding:10px 12px;">
                        <label style="color:var(--primary);">Qty in Stock UOM <span class="req">*</span></label>
                        <div style="display:flex;gap:6px;align-items:center;">
                            <asp:TextBox ID="txtQtyUOM" runat="server" placeholder="0" style="flex:1;border-color:rgba(74,127,165,0.4);background:#f0f6ff;" />
                            <asp:DropDownList ID="ddlStdUOM" runat="server" style="width:90px;padding:9px 6px;border:1.5px solid rgba(74,127,165,0.4);border-radius:8px;font-family:'DM Sans',sans-serif;font-size:13px;background:#f0f6ff;outline:none;" />
                        </div>
                    ...
                    <asp:Repeater ID="rptGRN" runat="server">
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td>
                                    <div style="font-weight:500;font-size:12px;"><%# Eval("StationaryName") %></div>
                                    <div style="font-size:10px;color:var(--text-dim);"><%# Eval("StationaryCode") %></div>
                                </td>
                        ...
                        <asp:Repeater ID="rptRecoverables" runat="server">
                            <ItemTemplate>
                                <div class="rec-item">
                                    ...
                                    <div class="rec-item-name"><%# Eval("StationaryName") %> <span style="font-size:10px;color:var(--text-dim);">(<%# Eval("StationaryCode") %>)</span></div>
