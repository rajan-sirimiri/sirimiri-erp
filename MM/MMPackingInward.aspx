<%@ Page Language="C#" AutoEventWireup="true" EnableEventValidation="false" Inherits="MMApp.MMPackingInward" %>
<!DOCTYPE html>
<html lang="en">
<head runat="server">
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Packing Material Inward -- MM</title>
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600&display=swap" rel="stylesheet" />
    <link rel="stylesheet" href="/StockApp/erp-tablet.css"/>
    <style>
        :root { --bg:#f5f5f5; --surface:#fff; --border:#e0e0e0; --accent:#cc1e1e; --teal:#1a9e6a; --blue:#1e78cc; --gold:#b8860b; --orange:#e07b00; --text:#1a1a1a; --text-muted:#666; --text-dim:#999; --radius:12px; }
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        input[type=text],input[type=date],input[type=number],select,textarea { width:100%; padding:9px 12px; border:1.5px solid var(--border); border-radius:8px; font-family:'DM Sans',sans-serif; font-size:13px; color:var(--text); background:#fafafa; transition:border-color .2s; outline:none; }
        ...
        .amt-row.total .val { font-family:'Bebas Neue',sans-serif; font-size:22px; color:var(--blue); }
        .check-row { display:flex; align-items:center; gap:10px; padding:8px 0; }
        .check-row input[type=checkbox] { width:15px; height:15px; accent-color:var(--blue); cursor:pointer; flex-shrink:0; }
        .check-row label { font-size:13px; font-weight:500; color:var(--text); text-transform:none; letter-spacing:0; cursor:pointer; }
        .qc-box { background:rgba(30,120,204,0.04); border:1.5px solid rgba(30,120,204,0.2); border-radius:8px; padding:12px 16px; }
        .qc-box .check-row label { font-size:13px; font-weight:600; color:var(--blue); }
        ...
        .btn-receive { background:var(--blue); color:#fff; }
        .btn-receive:hover { background:#155fa0; box-shadow:0 4px 12px rgba(30,120,204,0.3); }
        ...
        .alert-success { background:rgba(30,120,204,0.10); color:var(--blue); border:1px solid rgba(30,120,204,0.25); }
        ...
        .btn-filter { padding:6px 14px; background:var(--blue); color:#fff; border:none; border-radius:6px; font-size:12px; font-weight:700; cursor:pointer; }
        ...
        .badge-qc-pass  { display:inline-block; padding:2px 6px; border-radius:20px; font-size:10px; font-weight:700; background:rgba(30,120,204,0.08); color:var(--blue); }
...
<form id="form1" runat="server">
    ...
    <nav>
        ...
        <span class="nav-sep">&rsaquo;</span>
        <span class="nav-item active">Packing Inward</span>
        <div class="nav-right">
    ...
    <div class="page-header">
        <div>
            <div class="page-title">Packing Material <span>Inward</span></div>
            <div class="page-sub">Record goods received &mdash; generate GRN for packing materials</div>
        </div>
        <div class="grn-badge"><asp:Label ID="lblGRNNo" runat="server" Text="GRN-PM-#####" /></div>
    </div>
            ...
            <div class="card">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <asp:TextBox ID="txtHSN" runat="server" MaxLength="20" placeholder="Auto-filled from PM" />
                    </div>
                    <div class="form-group">
                    ...
                    <asp:Repeater ID="rptGRN" runat="server">
                        <HeaderTemplate>
                            <table class="grn-table"><thead><tr>
                                <th>GRN No</th><th>Date</th><th>PM Name</th><th>Supplier</th>
                        ...
                        <ItemTemplate>
                            <tr class="grn-row" data-grn='<%# Eval("GRNNo") %>'>
                                <td>
                                    <a href="javascript:void(0)" class="grn-no-link" onclick="toggleGRNDetail(this,'<%# Eval("GRNNo") %>','PM');">
                                ...
                                <td>
                                    <div style="font-weight:500;font-size:12px;"><%# Eval("PMName") %></div>
                                    <div style="font-size:10px;color:var(--text-dim);"><%# Eval("PMCode") %></div>
                                </td>
                        ...
                        <asp:Repeater ID="rptRecoverables" runat="server">
                            <ItemTemplate>
                                <div class="rec-item">
                                    ...
                                    <div class="rec-item-name"><%# Eval("PMName") %> <span style="font-size:10px;color:var(--text-dim);">(<%# Eval("PMCode") %>)</span></div>
...
<script>
    ...
    function showGRNConfirm() {
        ...
        var material = ddlText('<%= ddlPM.ClientID %>');
