...
<script>
    ...
    function loadRecoverablesAjax(supId) {
        ...
        fetch('MMRecoverablesAPI.ashx?supId=' + supId + '&type=RM')
    ...
    function onMaterialTypeahead(idx) {
        ...
        // Find first exact (case-insensitive) name match in rmOptions
        for (var i = 0; i < rmOptions.length; i++) {
        ...
        // No exact match yet — leave hidden select as-is (will resolve on full-name match)
    }
    ...
    function openMaterialSearch(idx) {
        ...
        // small delay to let the modal render before focusing (helps mobile keyboards)
    ...
    function renderMaterialList(filter) {
        ...
        for (var i = 0; i < rmOptions.length; i++) {
            ...
            html += '<div class="mat-row" onclick="applyMaterialPick(' + rmOptions[i].id + ');">'
                  + nm.replace(/</g, '&lt;') + '</div>';
            shown++;
            if (shown > 200) break; // safety cap to keep DOM small
        }
