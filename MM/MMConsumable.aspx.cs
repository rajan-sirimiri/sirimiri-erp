...
namespace MMApp
{
    public partial class MMConsumable : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!MMDatabaseHelper.RoleHasModuleAccess(__role, "MM", "MM_CM_MASTER"))
        ...
        {
            lblFormTitle.Text       = isEdit ? "Edit Consumable" : "New Consumable";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(name)) { ShowAlert("Consumable Name is required.", false); return; }
            if (ddlUOM.SelectedValue == "0") { ShowAlert("Please select a Unit of Measure.", false); return; }

            int     matId   = Convert.ToInt32(hfConsumableID.Value);
        ...
        protected void btnToggleActive_Click(object sender, EventArgs e)
        {
            int matId = Convert.ToInt32(hfConsumableID.Value);
            if (matId == 0) return;
            DataRow dr = MMDatabaseHelper.GetConsumableById(matId);
            if (dr == null) return;
            bool active = Convert.ToBoolean(dr["IsActive"]);
            MMDatabaseHelper.ToggleConsumableActive(matId, !active);
            ShowAlert("Consumable " + (active ? "deactivated" : "activated") + " successfully.", true);
            ClearForm();
            LoadMaterials();
        }
        ...
        protected void rptItems_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "Edit")
            {
                ...
                DataRow dr = MMDatabaseHelper.GetConsumableById(matId);
                if (dr == null) return;
                hfConsumableID.Value         = matId.ToString();
                txtCode.Text          = dr["ConsumableCode"].ToString();
                txtName.Text          = dr["ConsumableName"].ToString();
                ...
                pnlAlert.Visible      = false;
                LoadOpeningStock(matId);
                lblOSMaterialName.Text = dr["ConsumableName"].ToString();
            }
        }
        ...
        {
            DataRow dr = MMDatabaseHelper.GetOpeningStock("CN", matId);
            if (dr != null)
            {
        ...
        protected void btnSaveOS_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MMDatabaseHelper.SaveOpeningStock("CN", matId, qty, rate, asOfDate, txtOSRemarks.Text.Trim(), userId);
