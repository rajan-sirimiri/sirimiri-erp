<%@ Page Language="C#" AutoEventWireup="true" Inherits="FINApp.FINReceiptImport" EnableEventValidation="false" %>
<!DOCTYPE html>
<html lang="en">
<head runat="server">
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Sirimiri FIN — Receipt Import</title>
...
.tag-customer{background:#eafaf1;color:var(--teal);}
.tag-bank{background:#ebf5fb;color:var(--blue);}
.tag-internal{background:#fdf3f2;color:var(--red);}
.tag-other{background:#f5f5f5;color:var(--text-dim);}
...
<nav>
    ...
    <span class="nav-title">FIN — Receipt Register Import</span>
    <div class="nav-right">
...
<!-- UPLOAD -->
<div class="card">
    <div class="card-title">&#x1F4B0; Upload Tally Receipt Register</div>
    <div class="card-sub">Upload Receipts_Register.xlsx. Entries are auto-classified as Customer, Bank, Internal, or Other.</div>
...
<div class="card">
    <div class="card-title">&#x1F50D; Preview — Classification Breakdown</div>
    <div class="summary-bar">
        <div class="summary-stat"><div class="val"><asp:Label ID="lblTotal" runat="server" Text="0"/></div><div class="lbl">Total Rows</div></div>
        <div class="summary-stat"><div class="val" style="color:var(--teal);"><asp:Label ID="lblCustomer" runat="server" Text="0"/></div><div class="lbl">Customer</div></div>
        <div class="summary-stat"><div class="val" style="color:var(--blue);"><asp:Label ID="lblBank" runat="server" Text="0"/></div><div class="lbl">Bank</div></div>
        <div class="summary-stat"><div class="val" style="color:var(--red);"><asp:Label ID="lblInternal" runat="server" Text="0"/></div><div class="lbl">Internal</div></div>
        <div class="summary-stat"><div class="val" style="color:var(--text-dim);"><asp:Label ID="lblOther" runat="server" Text="0"/></div><div class="lbl">Other</div></div>
        <div class="summary-stat"><div class="val" style="color:var(--blue);"><asp:Label ID="lblSkipped" runat="server" Text="0"/></div><div class="lbl">Already Imported</div></div>
    </div>
    ...
    <asp:Panel ID="pnlUnmapped" runat="server" Visible="false">
        <div style="background:#fdf3f2;border:1px solid #f5c6cb;border-radius:10px;padding:14px;margin-bottom:14px;">
            <div style="font-size:12px;font-weight:700;color:var(--red);margin-bottom:8px;">&#x26A0; Unmapped Customers — these will be imported without CustomerID. Map them in Tally Mapping for full traceability.</div>
            <div style="max-height:200px;overflow-y:auto;">
                <asp:Repeater ID="rptUnmappedCustomers" runat="server">
                    <ItemTemplate><div class="unmapped-item"><%# Eval("Item") %></div></ItemTemplate>
                </asp:Repeater>
            </div>
        </div>
    </asp:Panel>
...
<div class="card">
    ...
    <asp:Repeater ID="rptImportHistory" runat="server">
        <HeaderTemplate>
            ...
            <thead><tr><th>Date</th><th>File</th><th>Inserted</th><th>Skipped</th><th>Errors</th><th>By</th></tr></thead><tbody>
        </HeaderTemplate>
        <ItemTemplate>
            <tr>
                ...
                <td style="color:var(--red);"><%# Eval("RowsError") %></td>
