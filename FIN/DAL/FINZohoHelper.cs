...
namespace FINApp.DAL
{
    ...
    public static class FINZohoHelper
    {
        ...
        // ══════════════════════════════════════════════════════════════
        //  GRN → ZOHO BILLS — purchase side
        // ══════════════════════════════════════════════════════════════
        // Each inward (raw/packing/consumable/stationery) is a GRN. For real
        // vendor purchases we push one GRN as one Zoho Bill. Internal stock
        // movements (GRN numbers starting with INT-/PREP-/PRE-) are excluded
        // by the FIN listing layer, so this helper trusts the caller.
        //
        // Auto-create policy (confirmed by user, session 14):
        //   - Vendor: if no zoho_suppliermap row, push the supplier first via
        //     the existing SyncSupplierToZoho flow.
        //   - Item: if no Zoho_RMItemMap / Zoho_PMItemMap row, create the item
        //     in Zoho with name + HSN + is_taxable, then cache the mapping.
        //
        // Bill granularity: ONE bill per GRN row. Multi-line-per-bill is TODO.
        // ══════════════════════════════════════════════════════════════

        /// <summary>Ensure a raw material is registered as a Zoho Books item. Returns the
        /// Zoho item_id. Creates on first call; returns cached id afterward.</summary>
        public static string EnsureRMItem(int rmId)
        {
            // Cached mapping first — avoids a Zoho round-trip on every push
            string existing = GetZohoRMItemId(rmId);
            if (!string.IsNullOrEmpty(existing)) return existing;

            var dt = new DataTable();
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "SELECT rm.RMID, rm.RMCode, rm.RMName, rm.Description, rm.HSNCode, rm.GSTRate, " +
                " u.Abbreviation AS Unit " +
                "FROM mm_rawmaterials rm " +
                "JOIN mm_uom u ON u.UOMID = rm.UOMID " +
                "WHERE rm.RMID = ?id;", conn))
            {
                cmd.Parameters.AddWithValue("?id", rmId);
                conn.Open(); dt.Load(cmd.ExecuteReader());
            }
            if (dt.Rows.Count == 0) throw new Exception("Raw material " + rmId + " not found");
            var r = dt.Rows[0];

            var item = new Dictionary<string, object>
            {
                { "name", r["RMName"].ToString() },
                { "sku", r["RMCode"].ToString() },
                ...
                // sales_and_purchases so the same Zoho item can be used on both
                // sides if the RM ever shows up on a sales invoice too.
                { "item_type", "sales_and_purchases" }
            };
            ...
            {
                string msg = result.ContainsKey("message") ? result["message"].ToString() : "Unknown error";
                LogSync("EnsureItem", "RM", rmId.ToString(), null, "Error", msg);
            ...
            string zohoItemId = zohoItem["item_id"].ToString();
            string zohoName = zohoItem["name"].ToString();
            SaveRMItemMap(rmId, zohoItemId, zohoName, "Synced");
            LogSync("EnsureItem", "RM", rmId.ToString(), zohoItemId, "Success",
                "Auto-created for GRN push: " + zohoName);
            return zohoItemId;
        }

        /// <summary>Ensure a packing material is registered as a Zoho Books item.
        /// Mirrors EnsureRMItem — creates on first call, caches mapping.</summary>
        public static string EnsurePMItem(int pmId)
        {
            string existing = GetZohoPMItemId(pmId);
            if (!string.IsNullOrEmpty(existing)) return existing;

            var dt = new DataTable();
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "SELECT pm.PMID, pm.PMCode, pm.PMName, pm.Description, pm.HSNCode, pm.GSTRate, " +
                " u.Abbreviation AS Unit " +
                "FROM mm_packingmaterials pm " +
                "JOIN mm_uom u ON u.UOMID = pm.UOMID " +
                "WHERE pm.PMID = ?id;", conn))
            {
                cmd.Parameters.AddWithValue("?id", pmId);
                conn.Open(); dt.Load(cmd.ExecuteReader());
            }
            if (dt.Rows.Count == 0) throw new Exception("Packing material " + pmId + " not found");
            var r = dt.Rows[0];

            var item = new Dictionary<string, object>
            {
                { "name", r["PMName"].ToString() },
                { "sku", r["PMCode"].ToString() },
                { "description", r["Description"] != DBNull.Value ? r["Description"].ToString() : "" },
                { "rate", 0 },
                { "unit", r["Unit"] != DBNull.Value ? r["Unit"].ToString() : "" },
                { "product_type", "goods" },
            ...
            {
                string msg = result.ContainsKey("message") ? result["message"].ToString() : "Unknown error";
                LogSync("EnsureItem", "PM", pmId.ToString(), null, "Error", msg);
                throw new Exception("Zoho item create failed: " + msg);
            }
            ...
            string zohoItemId = zohoItem["item_id"].ToString();
            string zohoName = zohoItem["name"].ToString();
            SavePMItemMap(pmId, zohoItemId, zohoName, "Synced");
            LogSync("EnsureItem", "PM", pmId.ToString(), zohoItemId, "Success",
                "Auto-created for GRN push: " + zohoName);
            return zohoItemId;
        }

        /// <summary>Ensure a supplier is in zoho_suppliermap. If not, trigger SyncSupplierToZoho
        /// which creates/updates in Zoho and upserts the mapping row. Returns Zoho contact_id.</summary>
        private static string EnsureVendor(int supplierId)
        {
            string existing = GetZohoSupplierId(supplierId);
            if (!string.IsNullOrEmpty(existing)) return existing;

            string syncResult = SyncSupplierToZoho(supplierId);
            if (!syncResult.StartsWith("Created") && !syncResult.StartsWith("Updated") && syncResult != "OK")
            {
                // Some older code paths return "Supplier not found" or "Error: ..."
                throw new Exception("Vendor sync failed: " + syncResult);
            }
            string after = GetZohoSupplierId(supplierId);
            if (string.IsNullOrEmpty(after))
                throw new Exception("Vendor sync returned OK but mapping row not found for supplier " + supplierId);
            return after;
        }

        /// <summary>Push one GRN to Zoho Books as a Bill. grnType must be RAW or PACKING for
        /// Phase 1; CONSUMABLE/STATIONERY return not-implemented. Idempotent: if already
        /// pushed (zoho_billlog row with non-empty ZohoBillID), returns the existing bill id.</summary>
        public static ZohoBillPushResult CreateBillFromGRN(int grnId, string grnType, int userId)
        {
            var res = new ZohoBillPushResult { GRNID = grnId, GRNType = grnType };
            try
            {
                ...
                // Already pushed? Look up existing bill
                string existingBillId = null;
                using (var conn = new MySqlConnection(ConnStr))
                using (var cmd = new MySqlCommand(
                    "SELECT ZohoBillID FROM zoho_billlog WHERE GRNID=?gid AND GRNType=?gt " +
                    "AND ZohoBillID IS NOT NULL AND ZohoBillID<>'' LIMIT 1;", conn))
                {
                    cmd.Parameters.AddWithValue("?gid", grnId);
                    cmd.Parameters.AddWithValue("?gt", grnType);
                    conn.Open();
                    var o = cmd.ExecuteScalar();
                    if (o != null && o != DBNull.Value) existingBillId = o.ToString();
                }
                if (!string.IsNullOrEmpty(existingBillId))
                {
                    res.Success = true;
                    res.AlreadyPushed = true;
                    res.ZohoBillID = existingBillId;
                    res.Message = "Already pushed to Zoho.";
                    return res;
                }

                // Read GRN row + material + supplier info from the appropriate inward table
                DataRow g;
                if (grnType == "RAW")
                ...
                int supplierId = Convert.ToInt32(g["SupplierID"]);
                res.SupplierID = supplierId;

                // Internal production is supplier 306 — refuse these at helper level too
                if (supplierId == 306) { res.Success = false; res.Message = "Internal Production GRNs cannot be pushed to Zoho."; return res; }

                // Vendor mapping (auto-create if missing)
                string zohoVendorId = EnsureVendor(supplierId);
                res.ZohoVendorID = zohoVendorId;

                // Item mapping (auto-create if missing)
                string zohoItemId;
                string itemNameForNotes;
                if (grnType == "RAW")
                {
                    int rmId = Convert.ToInt32(g["RMID"]);
                ...
                decimal rate = Convert.ToDecimal(g["Rate"]);
                string hsn = g["HSNCode"] != DBNull.Value ? g["HSNCode"].ToString() : "";
                string grnNo = g["GRNNo"].ToString();
                string invNo = g["InvoiceNo"] != DBNull.Value ? g["InvoiceNo"].ToString() : grnNo;

                // Bill line items — Phase 1 is exactly one line per bill
                var lineItem = new Dictionary<string, object>
                {
                    { "item_id", zohoItemId },
                    { "quantity", qty },
                ...
                DateTime billDate;
                if (g["InvoiceDate"] != DBNull.Value)
                    billDate = Convert.ToDateTime(g["InvoiceDate"]);
                else
                ...
                var serializer = new System.Web.Script.Serialization.JavaScriptSerializer();
                string billJson = serializer.Serialize(bill);
                LogSync("CreateBill_Debug", grnType + "GRN", grnId.ToString(), null, "Debug",
                    billJson.Length > 4000 ? billJson.Substring(0, 4000) : billJson);

                var result = ZohoPost("bills", bill);
                int code = Convert.ToInt32(result["code"]);
                if (code != 0)
                {
                    string msg = result.ContainsKey("message") ? result["message"].ToString() : "Unknown error";
                    SaveBillLog(grnId, grnType, supplierId, zohoVendorId, null, null, null,
                        billDate, Convert.ToDecimal(g["Amount"]), "Error", msg, userId);
                    LogSync("CreateBill", grnType + "GRN", grnId.ToString(), null, "Error", msg);
                    res.Success = false;
                    res.Message = msg;
                    return res;
                }

                var zBill = result["bill"] as Dictionary<string, object>;
                string zBillId = zBill["bill_id"].ToString();
                string zBillNo = zBill.ContainsKey("bill_number") ? zBill["bill_number"].ToString() : invNo;
                string zBillStatus = zBill.ContainsKey("status") ? zBill["status"].ToString() : "open";

                SaveBillLog(grnId, grnType, supplierId, zohoVendorId, zBillId, zBillNo, zBillStatus,
                    billDate, Convert.ToDecimal(g["Amount"]), "Pushed", null, userId);
                LogSync("CreateBill", grnType + "GRN", grnId.ToString(), zBillId, "Success",
                    "Bill " + zBillNo + " created for GRN " + grnNo);

                res.Success = true;
                res.ZohoBillID = zBillId;
                res.ZohoBillNo = zBillNo;
                res.Message = "Pushed as bill " + zBillNo;
                return res;
            }
            catch (Exception ex)
            {
                LogSync("CreateBill", grnType + "GRN", grnId.ToString(), null, "Error", ex.Message);
                res.Success = false;
        ...
        // ── GRN read helpers (return joined rows with supplier + material info) ──

        private static DataRow GetRawInwardRowForBill(int inwardId)
        {
            var dt = new DataTable();
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
        ...
        private static DataRow GetPackingInwardRowForBill(int inwardId)
        {
            var dt = new DataTable();
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
        ...
        public static string GetZohoRMItemId(int rmId)
        {
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "SELECT ZohoItemID FROM Zoho_RMItemMap WHERE RMID=?id LIMIT 1;", conn))
            {
                cmd.Parameters.AddWithValue("?id", rmId);
                conn.Open();
                var o = cmd.ExecuteScalar();
        ...
        public static string GetZohoPMItemId(int pmId)
        {
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "SELECT ZohoItemID FROM Zoho_PMItemMap WHERE PMID=?id LIMIT 1;", conn))
            {
                cmd.Parameters.AddWithValue("?id", pmId);
                conn.Open();
                var o = cmd.ExecuteScalar();
        ...
        private static void SaveRMItemMap(int rmId, string zohoItemId, string zohoName, string status)
        {
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "INSERT INTO Zoho_RMItemMap (RMID, ZohoItemID, ZohoItemName, SyncStatus, LastSyncAt) " +
                "VALUES(?id, ?zid, ?zn, ?st, NOW()) " +
                "ON DUPLICATE KEY UPDATE ZohoItemID=?zid, ZohoItemName=?zn, SyncStatus=?st, LastSyncAt=NOW();", conn))
            {
                cmd.Parameters.AddWithValue("?id", rmId);
        ...
        private static void SavePMItemMap(int pmId, string zohoItemId, string zohoName, string status)
        {
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "INSERT INTO Zoho_PMItemMap (PMID, ZohoItemID, ZohoItemName, SyncStatus, LastSyncAt) " +
                "VALUES(?id, ?zid, ?zn, ?st, NOW()) " +
                "ON DUPLICATE KEY UPDATE ZohoItemID=?zid, ZohoItemName=?zn, SyncStatus=?st, LastSyncAt=NOW();", conn))
            {
                cmd.Parameters.AddWithValue("?id", pmId);
        ...
        private static void SaveBillLog(int grnId, string grnType, int supplierId, string zohoVendorId,
            string zohoBillId, string zohoBillNo, string zohoStatus, DateTime billDate, decimal billTotal,
            string pushStatus, string errorMsg, int userId)
        {
            using (var conn = new MySqlConnection(ConnStr))
            using (var cmd = new MySqlCommand(
                "INSERT INTO zoho_billlog " +
                " (GRNID, GRNType, ErpSupplierID, ZohoBillID, ZohoVendorID, ZohoBillNo, BillDate, BillTotal, " +
                "  ZohoStatus, PushStatus, ErrorMessage, PushedAt, PushedBy) " +
                "VALUES(?gid, ?gt, ?sid, ?zbid, ?zvid, ?zbno, ?bd, ?bt, ?zs, ?ps, ?err, NOW(), ?uid) " +
                "ON DUPLICATE KEY UPDATE " +
                " ErpSupplierID=?sid, ZohoBillID=?zbid, ZohoVendorID=?zvid, ZohoBillNo=?zbno, BillDate=?bd, " +
                " BillTotal=?bt, ZohoStatus=?zs, PushStatus=?ps, ErrorMessage=?err, PushedAt=NOW(), PushedBy=?uid;", conn))
            {
                cmd.Parameters.AddWithValue("?gid", grnId);
                cmd.Parameters.AddWithValue("?gt", grnType);
                cmd.Parameters.AddWithValue("?sid", supplierId);
                cmd.Parameters.AddWithValue("?zbid", (object)zohoBillId ?? DBNull.Value);
                cmd.Parameters.AddWithValue("?zvid", (object)zohoVendorId ?? DBNull.Value);
                cmd.Parameters.AddWithValue("?zbno", (object)zohoBillNo ?? DBNull.Value);
                cmd.Parameters.AddWithValue("?bd", billDate);
                cmd.Parameters.AddWithValue("?bt", billTotal);
                cmd.Parameters.AddWithValue("?zs", (object)zohoStatus ?? DBNull.Value);
                cmd.Parameters.AddWithValue("?ps", pushStatus);
                cmd.Parameters.AddWithValue("?err", (object)errorMsg ?? DBNull.Value);
    ...
    /// <summary>Result of pushing one GRN to Zoho as a Bill.</summary>
    public class ZohoBillPushResult
    {
        public int GRNID;
