...
function addOLLine(){var sel=document.getElementById('selOLProduct');var qi=document.getElementById('txtOLQty');var pid=sel.value;if(pid==='0'){erpAlert('Please select a product.', {title:'Required', type:'warn'});return;}var qty=parseInt(qi.value)||0;if(qty<=0){erpAlert('Please enter a quantity.', {title:'Required', type:'warn'});return;}var opt=sel.options[sel.selectedIndex];var nm=productData[pid].name;var js=parseInt(opt.getAttribute('data-jarsize'))||1;var av=parseInt(opt.getAttribute('data-avail'))||0;for(var i=0;i<olLines.length;i++){if(olLines[i].pid===pid){erpAlert('This product is already added. Remove it first.', {title:'Duplicate', type:'warn'});return;}}if(qty>av){erpAlert('Only '+av+' available.', {title:'Stock Insufficient', type:'danger'});return;}olLines.push({pid:pid,name:nm,qty:qty,jarSize:js,pcs:qty*js});renderOLTable();qi.value='';sel.selectedIndex=0;document.getElementById('olAvailInfo').innerText='';}
