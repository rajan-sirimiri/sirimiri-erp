...
function onProductChange(sel){var v=sel.value;var info=document.getElementById('productInfo');var btn=document.getElementById('<%= btnPack.ClientID %>');if(!v||v==='0'){info.className='product-info';document.getElementById('calcBar').style.display='none';if(btn)btn.disabled=false;return;}var p=productData[v];if(!p){info.className='product-info';return;}var js=parseInt(p.unitSizes)||1,ap=parseInt(p.availPcs)||0,aj=Math.floor(ap/js),pc=parseInt(p.containersPerCase)||12,mx=Math.floor(aj/pc),ct=p.containerType||'JAR',cl=ct==='DIRECT'?'Containers':ct+'s';document.getElementById('piName').innerText=p.name;document.getElementById('piCode').innerText=p.code;document.getElementById('piAvailJars').innerText=aj.toLocaleString();document.getElementById('piAvailPcs').innerText=ap.toLocaleString();document.getElementById('piContainerLabel').innerText=cl+' Available';document.getElementById('piPerCase').innerText=pc;document.getElementById('piPerCaseLabel').innerText=cl+' per Case';document.getElementById('piMaxCases').innerText=mx;document.getElementById('txtUnitsPerCarton').value=pc;document.getElementById('txtCartons').setAttribute('max',mx);var pd=document.getElementById('casePMDisplay'),hf=document.getElementById('<%= hfCasePMID.ClientID %>');if(p.casePMID&&parseInt(p.casePMID)>0){pd.innerText=p.casePMName;pd.style.color='var(--text)';pd.style.fontWeight='600';hf.value=p.casePMID;}else{pd.innerText='No carton PM mapped';pd.style.color='var(--text-dim)';hf.value='0';}info.className='product-info show';calcSecondary();}
